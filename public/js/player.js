      ...
      <div class="embed-controls">
        <div class="form-group">
          ...
          <select class="form-select" id="embed-stream-select" onchange="updateEmbedPreview()">
            ...
            ${streams.map(s => `<option value="${s.id}" data-hls="${s.hls_url || ''}">${s.name} (${s.status})</option>`).join('')}
          </select>
        </div>
        <div class="form-row">
          <div class="form-group">
        ...
        ${liveStreams.length > 0 ? `<div style="margin-top:12px;padding:12px;background:rgba(16,185,129,0.08);border-radius:var(--radius-sm);border:1px solid rgba(16,185,129,0.2);">
          ...
          <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;">
            ${liveStreams.map(s => `<button class="btn btn-sm btn-success" onclick="openLivePreview('${s.id}','${s.name.replace(/'/g,"\\'")}')">📺 ${s.name}</button>`).join('')}
          </div>
        </div>` : ''}
      </div>
    </div>`;
